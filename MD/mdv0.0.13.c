...
{ 
  ...
  double *Etot,*Ecin,*Epot,*Tr,Pi,a,rho,Vinv,Pex,Pr,Ek,Cv,Tprom,N3,*P;
  int *semilla,gf;
    ...
    for(i=0;i<tsteps;i++)
     {
      verlet(posicion, v, fuerzas, dt, Vlj, Flj,  r, r2, dLt, N, L, rc2,P); 
      icount2=icount2+1;
      if(icount2%sfreq2==0){ // Esto es para tomar cada 100 pasos.
          icount3=icount3+1;
          ...
          *Tr=2.0*(*Ecin)/3.0;// Otra forma de ver la T.
          ...
          Pr+=rho*(*Tr);
          Tprom=Tprom+*Tr;
          Ek+=calc_Ek(v,N); //promedio de *Ecin a la -1.
          fprintf(fp,"%lf %lf \n",i*dt,*Etot); 
          fprintf(fp3,"%lf %lf \n",i*dt,*Ecin); 
          fprintf(fp4,"%lf %lf \n",i*dt,*Epot); 
          fprintf(fp5,"%lf %lf \n",i*dt,*Tr); 
     ...
     Cv=1.0/(N-N*Tprom*(N3-1.0)*Ek);
     fprintf(fp7,"Cv=%lf Tprom=%lf icoun3=%d Pr=%lf Ek=%lf \n",Cv,Tprom,\
     icount3,Pr,Ek);   
  fclose(fp);
  fclose(fp3);
  fclose(fp4);
