...
float calc_P(double *posicion, double *fuerzas, double *Vlj, double *Flj, \
double *r, double *r2, double dLt, double rc2, int N, double L);
...
//    Td=T;
    ...
    sprintf(filename,"Presion_L=%lf_N=%4.2d_T=%lf.dat",L,N,T);
    ...
    for(i=0;i<tsteps;i++)
     {
      ...
      if(icount2%sfreq2==0){ // Esto es para tomar cada 100 pasos.
          ...
          *Tr=2.0*(*Ecin)/3.0;// Otra forma de ver la T.
          Pex+=calc_P(posicion,fuerzas, Vlj, Flj, r, r2, dLt, rc2, N, L);
          Pr+=rho*(*Tr);
          Tprom=Tprom+*Tr;
          Ek+=calc_Ek(v,N);
...
double *r, double *r2, double dLt, double rc2, int N, double L)
{  int i,j,indice;
   double n2,Fx,Fy,Fz,Dx,Dy,Dz,dr,P; 
