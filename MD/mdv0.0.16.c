...
Esta version incluye el cálculo de la presión.
Es igual a v0.0.12, pero se supone que calcula la 
presión de forma mas eficiente*/
...
double *r, double *r2, double dLt, double rc2, int N, double L, double *P);

double deltax(double *posicion1, double *posicion2, double L);
void verlet(double *posicion, double *v, double *fuerzas, double dt, \
double *Vlj,double *Flj, double *r, double *r2, double dLt, int N, double L,\
 double rc2, double *P);
...
float calc_Ek(double *v, int N);

int main (int argc, char *argv[])
{ 
  int i,N,sfreq,sfreq2,tsteps,icount2,icount3;
  double *posicion,*v,*Vlj,*Flj,*r,*r2,*fuerzas,n,rc2, dt,L,tmax,T,Td,dLt;
  double *Etot,*Ecin,*Epot,*Tr,Pi,a,rho,Vinv,Pex,Pr,Ek,Cv,Tprom,N3,*P,Ek2;
  int *semilla,gf;
...
//  rho=N/(L*L*L);
 ...
 for(T=2.0;T>=0.4;T=T-0.2)
   { 
    Td=T;
  
    char filename[255]; 
    sprintf(filename,"Etot_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp=fopen(filename,"w");

    sprintf(filename,"Ecin_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp3=fopen(filename,"w");
 
    sprintf(filename,"Epot_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp4=fopen(filename,"w");

    sprintf(filename,"Treal_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp5=fopen(filename,"w");

    sprintf(filename,"Lambda_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp6=fopen(filename,"w");

    sprintf(filename,"Presion_L=%lf_N=%4.2d_T=%lf.dat",L,N,Td);
    FILE *fp7=fopen(filename,"w");
 
    *P=0.0;
    qlfta(fuerzas, N);   
    interaccion(posicion,fuerzas,Vlj,Flj,r,r2,dLt,rc2,N,L,P); 
    Pex=0.0;
    Pr=0.0;
    N3=2.0/(3.0*N); //3.0*N/2.0;
    icount2=0;
    icount3=0;
    Ek=0.0;
    Ek2=0.0;  
    ...
    i=0;
    *Tr=calc_T(v,N);  
    printf("Tr= %lf in %d steps \n",*Tr,i);    
  

/* Acá dejamos que reescalee en un lapso de tiempo en el cual 
   llega a termalizar.*/
    for(i=1;i<2*tsteps;i++)
     {
      verlet(posicion, v, fuerzas, dt, Vlj, Flj,  r, r2, dLt, N, L, rc2,P);       
      *Tr=calc_T(v,N);   
       if(i%sfreq==0) escaleo_v(v,N,Tr,Td); 
      }

/* Esto es para corroborar que el sistema llego a la temperatura
   deseada.*/
    printf("Tr= %lf in %d steps \n",*Tr,i);    

/* Ahora viene la parte donde vamos a medir.*/
    for(i=0;i<tsteps;i++)
     {
      verlet(posicion, v, fuerzas, dt, Vlj, Flj,  r, r2, dLt, N, L, rc2,P); 
      if(i%sfreq2==0){ // Esto es para tomar cada 100 pasos.
          icount3=icount3+1;
          hamiltoniano(posicion,v,fuerzas,dt,Vlj,Flj,r,r2,dLt,rc2,N,L,\
          Etot,Ecin,Epot);
          *Tr=2.0*(*Ecin)/3.0;// Otra forma de ver la T.
          Pex+=*P;
          Pr+=rho*(*Tr);
          Tprom=Tprom+*Tr;
          Ek+=*Ecin; //promedio de *Ecin a la -1.
          Ek2+=(*Ecin)*(*Ecin);
          fprintf(fp,"%lf %lf \n",i*dt,*Etot); 
          fprintf(fp3,"%lf %lf \n",i*dt,*Ecin); 
          fprintf(fp4,"%lf %lf \n",i*dt,*Epot); 
     ...
     Cv=1.0/(N3-N3*N3*Ek/(Tprom*Tprom));
     Cv=Cv/N;
     fprintf(fp7,"Cv=%lf Tprom=%lf icoun3=%d Pr=%lf Ek=%lf rho=%lf \n",Cv,Tprom,\
     icount3,Pr,Ek,rho);   
  fclose(fp);
  fclose(fp3);
  fclose(fp4);
...
{ double vx2,Ek;
  int i;
   ...
   for (i=0; i<N;i++)
   {   
    vx2+=2.0/(*(v+3*i)*(*(v+3*i))+(*(v+3*i+1)*(*(v+3*i+1)))+\
           (*(v+3*i+2)*(*(v+3*i+2))));
   }
   Ek=vx2/N;
   return (double)Ek;
...
double *Flj, double *r, double *r2, double dLt, int N, double L, double rc2, double *P)
{ 
   int i;
