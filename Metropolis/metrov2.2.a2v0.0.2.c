...
void flipeo (int *red, int dim, float Jint, float B,  int *semilla,float *pacep, FILE *fp, FILE *fp2);
void aceptacion (int *red, int dim, float Delta, int i, int j, float Jint, float B, int *semilla, float *pacep, FILE *fp, FILE *fp2);
...
//  fp2=fopen("MMCMag.dat","w");
  ...
  for(i=0; i<N;i++)
   {
//   cborde(red,dim);
   flipeo (red,dim,Jint,B,semilla,pacep,fp,fp2); 
//   cborde(red,dim); 
   }
//   cborde(red,dim);
  imprimir(red,dim);
   ...
   for(i=1;i<dim-1;i++)
    {
     ...
     aceptacion(red,dim,Delta,i,j,Jint,B,semilla,pacep,fp,fp2);
    }
   }
}   
...
{     float p;
      ...
      *pacep=*pacep+1;
      ...
      fprintf(fp2," %f %f \n", *pacep, magnetizacion(red,dim));
      fprintf(fp,"%f %f \n", *pacep, hamiltoniano(red,dim,Jint,B)); 
      }
}
