  ...
  *semilla=S;
  ...
  for(T=2.0;T>=0.5;T=T-0.15)
   { 
    ...
    for(i=0;i<tsteps;i++)
     {
/*Hacer subiteracion para promediar la presion antes de reescalear las 
velocidades. Puedo tomar dt mas grande, 10^-2.
Hay que dejar termalizar cada vez que se reescalea. Puedo usar el sfreq*/
...
//   imprimir(posicion,N,L,i*dt);
      ...
      *Tr=calc_T(v,N);
      if(*Tr>Td && i%sfreq==0) escaleo_v(v,N,Tr,Td);   
