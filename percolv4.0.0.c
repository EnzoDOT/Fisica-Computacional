...
{ int *red,*ns,*ns2,*historial;
  ...
  int per;
  int i,j,k,N,Pinf;
  float p,pc,tau,sigma,aux1,aux2;
  int *semilla,*cper;
  ...
  sigma=(1.0)*9/25;
   ...
   sprintf(filename,"scaling_L=%d.dat",dim);
   FILE *fp=fopen(filename,"w");
   fp2=fopen("Scaling.dat","w"); 
   red=(int*) malloc(dim*dim*sizeof(int));  
   ns=(int*) malloc(dim*dim*sizeof(int));         
   ns2=(int*) malloc(dim*dim*sizeof(int));    
   nspc=(int*) malloc(dim*dim*sizeof(int));    
